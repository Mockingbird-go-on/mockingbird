...
def kb_entries():
    """Load (filename, parsed-dict) for every YAML in the bundled KB dir."""
    assert _KB_DIR.is_dir(), f"KB asset missing: {_KB_DIR}"
    entries: list[tuple[str, dict]] = []
    for path in sorted(_KB_DIR.glob("*.yaml")):
        with open(path, encoding="utf-8") as fh:
            data = yaml.safe_load(fh)
        entries.append((path.name, data if isinstance(data, dict) else {}))
    return entries


def test_kb_dir_exists():
    assert _KB_DIR.is_dir() and any(_KB_DIR.glob("*.yaml"))
